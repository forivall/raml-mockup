  ...
  _.each(api.resources, function(resource, path) {
    ...
    _.each(resource, function(responses, method) {
      ...
      app.route(route)[method](function(req, res) {
        ...
        var reqForceExample = req.query._forceExample === 'true' || params.forceExample || req.headers['x-mock-force-example'] === 'true' || options.forceExample;
        var reqExample = req.query.example || 'default';
        ...
        try {
          ...
          var sample = responses[reqStatus].examples[reqExample] || responses[reqStatus].example || null;
